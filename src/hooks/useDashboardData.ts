...
import { predictTomorrowAI, predictMonthlyAI, generateInsights, predictGoalProbability, recommendBestWorkDays } from "@/utils/aiPrediction";
...
export interface DashboardData {
  ...
  goalProbability: ReturnType<typeof predictGoalProbability>;
  bestWorkDays: ReturnType<typeof recommendBestWorkDays>;
}
...
export function useDashboardData(realWeather: Weather): DashboardData {
  ...
  const tomorrowPrediction = useMemo(
    () => predictTomorrowAI(records, realWeather, settings),
    [records, realWeather, settings]
  );
  ...
  const goalProbability = useMemo(
    () => predictGoalProbability(records, settings),
    [records, settings]
  );

  const bestWorkDays = useMemo(
    () =>
      recommendBestWorkDays(
        records,
        [realWeather, "sunny", "sunny", "cloudy", "rainy", "sunny", "cloudy"],
        settings.basePrice,
        settings
      ),
    [records, realWeather, settings]
  ...
  const dataHealth = useMemo(() => {
    ...
    const dataQualityScore = Math.min(100, Math.round(
      50 + Math.min(25, totalRecords * 0.5) + coverage30Percent * 0.2 + Math.min(20, maxStreak * 2)
    ));
    const statusLabel = dataQualityScore >= 90 ? "优秀" : dataQualityScore >= 70 ? "良好" : dataQualityScore >= 50 ? "一般" : "待提升";
    const statusColor = dataQualityScore >= 90 ? "#00E676" : dataQualityScore >= 70 ? "#00E5FF" : dataQualityScore >= 50 ? "#FFD740" : "#FF1744";

    return {
      totalRecords, coveragePercent, coverage30Percent,
      dataQualityScore, visible: totalRecords >= 3,
      statusLabel, statusColor,
      todayHasData: !!records[today()]?.orders, maxStreak,
    };
  ...
  return {
    ...
    efficiencyData, dataHealth, goalProbability, bestWorkDays,
  };
}
