import { motion, AnimatePresence } from "framer-motion";
import { X } from "lucide-react";
import { useEffect } from "react";

interface BottomSheetProps {
  isOpen: boolean;
  onClose: () => void;
  title: string;
  children: React.ReactNode;
  headerAction?: React.ReactNode;
  ios?: boolean;
}

export default function BottomSheet({
  isOpen,
  onClose,
  title,
  children,
  headerAction,
  ios,
}: BottomSheetProps) {
  useEffect(() => {
    if (isOpen) {
      document.body.classList.add("sheet-open");
      document.body.style.overflow = "hidden";
    } else {
      document.body.classList.remove("sheet-open");
      document.body.style.overflow = "";
    }
    return () => {
      document.body.classList.remove("sheet-open");
      document.body.style.overflow = "";
    };
  }, [isOpen]);

  return (
    <AnimatePresence>
      {isOpen && (
        <>
          <motion.div
            className="fixed inset-0 z-40"
            style={{ background: ios ? "rgba(0,0,0,0.25)" : "rgba(0,0,0,0.7)" }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={onClose}
          />
          <motion.div
            className={`fixed bottom-0 left-0 right-0 z-50 max-h-[80vh] max-h-[80dvh] overflow-y-auto overscroll-contain ${
              ios ? "ios-sheet rounded-t-[28px]" : "rounded-t-[28px]"
            }`}
            style={
              ios
                ? { WebkitOverflowScrolling: "touch" }
                : {
                    background: "rgba(4, 6, 16, 0.92)",
                    backdropFilter: "blur(12px)",
                    WebkitBackdropFilter: "blur(12px)",
                    borderTop: "1px solid rgba(0, 229, 255, 0.1)",
                    borderLeft: "1px solid rgba(0, 229, 255, 0.04)",
                    borderRight: "1px solid rgba(0, 229, 255, 0.04)",
                    boxShadow: "0 -8px 40px rgba(0,0,0,0.6), 0 -2px 8px rgba(0,0,0,0.4)",
                    WebkitOverflowScrolling: "touch",
                  }
            }
            initial={{ y: "100%" }}
            animate={{ y: 0 }}
            exit={{ y: "100%" }}
            transition={{ type: "spring", damping: 25, stiffness: 300 }}
          >
            {/* Top highlight line */}
            {!ios && (
              <div className="absolute top-0 left-[15%] right-[15%] h-[0.5px] pointer-events-none z-10"
                style={{
                  background: "linear-gradient(90deg, transparent, rgba(0,229,255,0.2), rgba(0,229,255,0.3), rgba(0,229,255,0.2), transparent)",
                }}
              />
            )}
            {ios && (
              <div className="flex justify-center pt-2 pb-1">
                <div className="w-10 h-1 rounded-full bg-black/10" />
              </div>
            )}
            <div
              className={`sticky top-0 z-10 flex items-center justify-between p-5 pb-2 ${
                ios ? "" : ""
              }`}
              style={
                ios
                  ? { background: "transparent" }
                  : {
                      background: "rgba(4, 6, 16, 0.92)",
                      backdropFilter: "blur(12px)",
                      WebkitBackdropFilter: "blur(12px)",
                    }
              }
            >
              <div className="w-8 flex items-center justify-start">
                {headerAction}
              </div>
              <h3 className={`text-lg font-bold tracking-tight ${ios ? "text-black" : "text-[#E0E0E0]"}`}>
                {title}
              </h3>
              <button
                onClick={onClose}
                className={`w-8 h-8 flex items-center justify-center rounded-full ${
                  ios ? "bg-[#F2F2F7] text-[#8E8E93]" : "tap-cyber"
                }`}
                style={
                  ios
                    ? undefined
                    : {
                        background: "rgba(0,229,255,0.05)",
                        border: "1px solid rgba(0,229,255,0.1)",
                      }
                }
              >
                <X size={16} className={ios ? "" : "text-[#E0E0E0]/40"} />
              </button>
            </div>
            <div className="px-5 pb-8">{children}</div>
          </motion.div>
        </>
      )}
    </AnimatePresence>
  );
}